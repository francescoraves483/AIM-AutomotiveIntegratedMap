...
json11::Json::object JSONserver::make_AIM_json(double lat, double lon) {
	...
	for(ldmmap::LDMMap::returnedVehicleData_t vehdata : returnedvehs) {
		...
		if(m_enhanced_CAMs_enabled==false) {
			...
			vehicles.push_back(make_vehicle_enhanced(vehdata.vehData.stationID,
				vehdata.vehData.lat,
				vehdata.vehData.lon,
				vehdata.vehData.exteriorLights.isAvailable() ? exteriorLights_bit_to_string(vehdata.vehData.exteriorLights.getData()) : "unavailable",
				vehdata.vehData.camTimestamp,
				...
				now_us-vehdata.vehData.timestamp_us,
				vehdata.vehData.heading,
				vehdata.vehData.rssi_dBm,
				vehdata.vehData.rssi_auxiliary_dBm,
				vehdata.vehData.cpu_load_perc,
				vehdata.vehData.ram_load_MB,
				vehdata.vehData.gpu_load_perc,
				vehdata.vehData.extradev_cpu_load_perc,
				vehdata.vehData.extradev_ram_load_MB,
				vehdata.vehData.extradev_gpu_load_perc,
				vehdata.vehData.ipaddr,
				vehdata.vehData.publicipaddr
				));
		}
	}
...
json11::Json::object JSONserver::make_vehicle_enhanced(uint64_t stationID, 
	double lat, 
	double lon, 
	...
	vehicle["rssi_dBm"] = MAKE_NUM(rssi);
	vehicle["auxilliary_rssi_dBm"] = MAKE_NUM(aux_rssi);
	vehicle["cpu_load_perc"] = MAKE_NUM(cpu_load_perc);
	vehicle["ram_load_MB"] = MAKE_NUM(ram_load_MB);
	vehicle["gpu_load_perc"] = MAKE_NUM(gpu_load_perc);
	vehicle["extradev_cpu_load_perc"] = MAKE_NUM(extradev_cpu_load_perc);
	vehicle["extradev_ram_load_MB"] = MAKE_NUM(extradev_ram_load_MB);
	vehicle["extradev_gpu_load_perc"] = MAKE_NUM(extradev_gpu_load_perc);
	vehicle["ipaddr"] = MAKE_NUM(ipaddr);
	vehicle["publicipaddr"] = MAKE_NUM(publicipaddr);
